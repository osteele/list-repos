...
### Changed
- The TUI is now the automatic default when stdin and stdout are terminals; redirected streams, report-specific flags, bulk operations, `--list`, and a configured list default retain line-oriented output (2026-08-26)
- TUI: expanded key help is a modal overlay that intercepts background keys instead of adding lines that displace the repository list (2026-08-26)
- TUI: the short key-help line is anchored to the bottom row instead of moving as directories expand and collapse (2026-08-26)
- TUI: bulk actions use active status verbs and report real queued/running/completed repository transitions plus elapsed time; failures automatically open a pageable per-repository error report retained on a selected directory (2026-08-26)
- TUI: all confirmation prompts use sentence capitalization and wrap within the terminal, with layout space reserved for every wrapped line (2026-08-26)
- TUI: bulk failure reports now separate and label the repository name, abbreviated location, VCS, and error output under numbered failure headings (2026-08-26)
- TUI: recursive directory rollups with nested repositories now show their arithmetic, such as `42 repos (31 direct + 11 nested)`, so the total is not mistaken for an immediate-subdirectory count (2026-08-26)
- Jujutsu fix results now report revisions changed across repositories changed, derived from revision identities before and after each fix operation (2026-08-26)
...
### Fixed
- Jujutsu ahead status now counts only revisions reachable from publishable local state and absent from remote bookmarks, rather than unrelated visible history; push tugs the closest bookmark first and verifies the resulting state so a refusal with exit status zero is not reported as success (2026-08-26)
- TUI: commit, push, pull, and sync results now invalidate and recompute affected directory rollups and the total; late pre-action scans cannot restore stale counts (2026-08-26)
- TUI: lowercase `p`, `u`, `c`, and `s` on a directory rollup now plan the operation only for eligible repositories beneath that selected directory; uppercase bulk actions remain global to the scan root and recursively include collapsed rollups (2026-08-26)
- TUI: `f` runs `jj fix` for a selected Jujutsu repository or as a confirmed, sequential plan beneath a selected directory; reveal-in-file-manager moves to `v` (2026-08-26)
...
### Added
- TUI: the AI change-description display combines description, summary, and files when they fit; otherwise it provides independently pageable Description/Summary/Files views with a pinned model/tool/target line (2026-08-26)
- TUI: AI descriptions generated by either `c` or `d` are cached in memory per repository and VCS diff hash; canceling keeps the draft warm for either view, while a changed tree, successful commit, or session exit invalidates it (2026-08-26)
- TUI: the `c` prompt is an expanding multiline editor that displays the complete generated commit subject and body when terminal space permits (2026-08-26)
- TUI: the AI change-description view offers `c` to commit with the generated description and return to the repository list, alongside `esc` to return without committing (2026-08-26)
- TUI: `d` runs `git-ai-commit --dry-run --json` or `jj-ai-commit --dry-run --json` for a dirty repository, validates the `ai-describe/v1` contract, and opens the structured result without modifying the repository (2026-08-26)
- XDG configuration at `gitsync/config.toml` can set `default_mode` to `tui` or `list`; the schema rejects unknown keys and invalid modes (2026-08-26)
